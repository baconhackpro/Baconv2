...
spawn(function() 
    ...
    wait(.5)
    require(game.ReplicatedStorage.Notification).new("<Color=Red>Thì Bình Nguyễn đã trở lại!!!!!<Color=/>"):Display()
    wait(.14)
...
local Window = OrionLib:MakeWindow({Name = "Bacon Hack Pro", HidePremium = false, IntroText = "NightHub Library", SaveConfig = true, ConfigFolder = "Bacon Hub"})
...
OrionLib:MakeNotification({
	Name = "Bacon Hub",
	Content = "Binh Nguyen Not Skidder, Please don't says",
	Image = "rbxassetid://4483345998",
	Time = 25
})
...
pcall(function()
    while wait() do
        if _G.AutoEliteHunter then
            if game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == true then
                OrionLib:MakeNotification({
                    Name = "Bacon Hack Pro(Noitification)",
